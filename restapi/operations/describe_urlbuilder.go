...
func (o *DescribeURL) Build() (*url.URL, error) {
	...
	var _path = "/v1/describe/stacks/{stackName}/versions/{version}"

	stackName := o.StackName
	if stackName != "" {
		_path = strings.Replace(_path, "{stackName}", stackName, -1)
	} else {
		return nil, errors.New("stackName is required on DescribeURL")
	}
	...
	if version != "" {
		...
		return nil, errors.New("version is required on DescribeURL")
	}
...
func (o *DescribeURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DescribeURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DescribeURL")
	}
